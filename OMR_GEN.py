...
# Configurations for 30-question OMR (Classes 1, 2, 3)
ROLL_X_CM_30 = [7.20, 7.82, 8.44, 9.06, 9.68]
BUBBLE_Y_TOP_CM_30 = [18.8] * 5
BUBBLE_SPACING_CM_30 = 0.612
BUBBLE_RADIUS_CM = 0.23
OMR_TEMPLATE_30 = "PHN_Unique_ID_1.jpg"  # Replace with actual 30-question OMR template file
 
# Configurations for 50-question OMR (Classes > 3)
ROLL_X_CM_50 = [4.6, 5.22, 5.84, 6.46, 7.08]
BUBBLE_Y_TOP_CM_50 = [19.45] * 5
BUBBLE_SPACING_CM_50 = 0.622
OMR_TEMPLATE_50 = "PHN_Unique_ID_2.jpg"  # Replace with actual 50-question OMR template file
 
EXCEL_FILE = "OMR_GEN.xlsx"
OUTPUT_DIR = "Generated_OMRs_1"
...
try:
    ...
    print(f"ERROR: Cannot read Excel file: {EXCEL_FILE}", e)
    exit()
...
for sheet_name in xls.sheet_names:
    ...
    # Group students by class for combined PDFs
    grouped = df.groupby(col_map["class"]) if col_map["class"] else None
    if grouped is None:
        print("No 'Class' column found; cannot group by class.")
        continue
 
    for class_val, group in grouped:
        # Determine class_numeric similarly
        class_numeric = None
        try:
        ...
        pdf_filename = os.path.join(
            OUTPUT_DIR, f"{safe_filename(sheet_name)}_Class_{safe_filename(str(class_val))}.pdf"
        )
        c = canvas.Canvas(pdf_filename, pagesize=A4)
        width, height = A4
 
        if not os.path.exists(omr_template):
            print(f"ERROR: OMR template not found: {omr_template}")
            continue
        ...
        c.save()
        print(f"Saved combined PDF for class {class_val}: {pdf_filename}")
 
print("All done! Check the Generated_OMRs folder.")
 
# combined code generates omr class wise
...
# After your PDF generation code finishes, call this function:
 
zip_output_file = "OMR_Classwise_PDFs.zip"
create_zip_of_pdfs(OUTPUT_DIR, zip_output_file)
 
# makes a zip of all the omrs (execute only after all pdfs are created)